...
TEST_CASE("Check Game starts with a player object") {
	Game newGame(1);
	CHECK( newGame.getGOVector()[0].getObjectType() == gameObjectType::Player);
}

TEST_CASE("Check Player is instantiated at (700,400)"){
	Game newGame(1);
	CHECK(newGame.getGOVector()[0].getXCoord() == 700);
	CHECK(newGame.getGOVector()[0].getYCoord() == 400);
}


TEST_CASE("Check player can move in anti-clockwise direction"){
	Game newGame(5);
	int oldx = newGame.getGOVector()[0].getXCoord(), oldy = newGame.getGOVector()[0].getYCoord();
	newGame.movePlayerObject(1);
	int newx = newGame.getGOVector()[0].getXCoord(), newy = newGame.getGOVector()[0].getYCoord();
	CHECK(oldx > newx);
	CHECK(oldy < newy);
}
...
TEST_CASE("Check bullet can be instantiated"){
	Game newGame(5);
	int oldSize = newGame.getGOVector().size();
	newGame.AddGameObject(gameObjectType::PlayerBullet);
	CHECK(newGame.getGOVector().size() == oldSize + 1);
}

TEST_CASE("Check bullet is of type 'PlayerBullet'"){
	Game newGame(5);
	newGame.AddGameObject(gameObjectType::PlayerBullet);
	CHECK(newGame.getGOVector()[newGame.getGOVector().size()-1].getObjectType() == gameObjectType::PlayerBullet);
}
// Check bullet move
...
TEST_CASE("Check multiple enemies can be instantiated"){
	...
	newGame.AddGameObject(gameObjectType::Enemy);
	newGame.AddGameObject(gameObjectType::Enemy);
	CHECK(newGame.getGOVector().size() == oldSize + 3);
}